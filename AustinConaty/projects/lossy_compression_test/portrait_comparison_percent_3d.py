...
Third panel shows PERCENT DIFFERENCE: 100 * (File 1 - File 2) / File 2
Handles 2D, 3D, and 4D data automatically
Treats values >= 1e+15 as undefined/missing data
"""
...
# Global undefined data threshold
UNDEFINED_VALUE = 1e15
...
def mask_undefined_data(data, undefined_value=UNDEFINED_VALUE):
    """
    Mask undefined data values
    ...
    undefined_value : float
        Value representing undefined/missing data
    
    Returns:
    --------
    masked_data : masked array
        Data with undefined values masked
    """
    # Convert to masked array if not already
    if not isinstance(data, np.ma.MaskedArray):
        data = np.ma.array(data)
    
    # Mask values >= undefined_value
    data = np.ma.masked_greater_equal(data, undefined_value)
    
    # Also mask inf and nan
...
def plot_three_panel_comparison(data1, data2, lon=None, lat=None,
    ...
    symmetric_diff : bool
        If True, use symmetric color scale for percent difference plot
    percent_diff_limit : float, optional
        If provided, use this as the symmetric limit for percent difference (e.g., 10 for ±10%)
    """
    
    # Ensure data is masked (should already be done in extract_horizontal_slice)
    data1 = mask_undefined_data(data1)
    data2 = mask_undefined_data(data2)
    
    # Calculate percent difference: 100 * (data1 - data2) / data2
    # Handle division by zero and near-zero values
    with np.errstate(divide='ignore', invalid='ignore'):
        pct_diff = 100.0 * (data1 - data2) / data2
    
    # Mask invalid values (inf, -inf, nan)
    pct_diff = np.ma.masked_invalid(pct_diff)
    
    # Also mask where data2 is very close to zero (avoid extreme percent differences)
    threshold = 1e-10
    pct_diff = np.ma.masked_where(np.abs(data2) < threshold, pct_diff)
    
    # Mask undefined values in percent difference
    pct_diff = mask_undefined_data(pct_diff)
    ...
    valid_pct_diff = pct_diff.compressed() if hasattr(pct_diff, 'compressed') else pct_diff[~np.isnan(pct_diff)]
    ...
    cmap_diff = 'RdBu_r'  # Always use diverging colormap for percent differences
    ...
    # For percent difference plot
    if len(valid_pct_diff) > 0:
        if percent_diff_limit is not None:
            # Use user-specified limit
            vmax_pct = percent_diff_limit
            vmin_pct = -percent_diff_limit
        elif symmetric_diff:
            # Make symmetric around zero
            vmax_pct = max(abs(np.min(valid_pct_diff)), abs(np.max(valid_pct_diff)))
            vmin_pct = -vmax_pct
        else:
            vmin_pct = np.min(valid_pct_diff)
            vmax_pct = np.max(valid_pct_diff)
    else:
        vmin_pct = -10
        vmax_pct = 10
    ...
    if use_cartopy and lon is not None and lat is not None:
        try:
            ...
            # Panel 3: Percent Difference (bottom)
            ax3 = plt.subplot(3, 1, 3, projection=ccrs.PlateCarree())
            setup_cartopy_axis(ax3)
            im3 = ax3.pcolormesh(lon, lat, pct_diff, 
                                transform=ccrs.PlateCarree(),
                                cmap=cmap_diff, vmin=vmin_pct, vmax=vmax_pct,
                                shading='auto')
            
            panel_title3 = 'Percent Difference: 100×(File 1 - File 2)/File 2'
            if level_info:
            ...
            # Add colorbar for percent difference panel
            cbar3 = plt.colorbar(im3, ax=ax3, orientation='horizontal', 
                                pad=0.05, shrink=0.8)
            cbar3.set_label("Percent Difference (%)", fontsize=10)
            
            # Add percent difference statistics text
            if len(valid_pct_diff) > 0:
                abs_diff = np.abs(data1 - data2)
                valid_abs_diff = abs_diff.compressed() if hasattr(abs_diff, 'compressed') else abs_diff[~np.isnan(abs_diff)]
                rmse = np.sqrt(np.mean(valid_abs_diff**2)) if len(valid_abs_diff) > 0 else 0
                mean_pct = np.mean(valid_pct_diff)
                stats3 = f"min={np.min(valid_pct_diff):.2f}%, max={np.max(valid_pct_diff):.2f}%, mean={mean_pct:.2f}%, RMSE={rmse:.3e}"
            else:
    ...
    if not use_cartopy:
        ...
        if lon is not None and lat is not None:
            ...
            im2 = axes[1].pcolormesh(lon, lat, data2, cmap=cmap_data, 
                                     vmin=vmin_data, vmax=vmax_data, shading='auto')
            im3 = axes[2].pcolormesh(lon, lat, pct_diff, cmap=cmap_diff, 
                                     vmin=vmin_pct, vmax=vmax_pct, shading='auto')
            for ax in axes:
            ...
            im2 = axes[1].imshow(data2, cmap=cmap_data, vmin=vmin_data, vmax=vmax_data,
                                origin='lower', aspect='auto')
            im3 = axes[2].imshow(pct_diff, cmap=cmap_diff, vmin=vmin_pct, vmax=vmax_pct,
                                origin='lower', aspect='auto')
            for ax in axes:
...
def compare_two_files(dataset1, dataset2, 
    ...
    symmetric_diff : bool
        If True, use symmetric color scale for percent difference plots
    ...
    percent_diff_limit : float, optional
        If provided, use this as symmetric limit for percent difference (e.g., 10 for ±10%)
    """
    ...
    print(f"Undefined data threshold: {UNDEFINED_VALUE:.2e}")
    print("="*70)
        ...
        try:
            ...
            for t_idx in time_indices:
                for l_idx in level_indices:
                    ...
                    # Data should already be masked from extract_horizontal_slice
                    # but ensure it's masked array
                    data1 = mask_undefined_data(data1)
                    data2 = mask_undefined_data(data2)
                    ...
                    plot_three_panel_comparison(data1, data2, lon, lat,
                                               ...
                                               percent_diff_limit=percent_diff_limit)
...
def main():
    """
    Main function - compare two NetCDF files with PERCENT DIFFERENCE plots
    Undefined data values >= 1e+15 are masked
    """
...
#    netcdf_file2 = "GEOS.fp.fcst.inst3_3d_chm_Np.20251202_00+20251210_0600.compress.nc4"      # Path to second NetCDF file
    ...
    # Output settings
    output_dir = "comparison_plots_pctdiff"  # Directory to save plots
    ...
    symmetric_diff = True          # Use symmetric color scale for percent difference plots
    percent_diff_limit = None      # Set to a number (e.g., 10) to fix scale at ±10%, or None for auto
    
    # ===========================================
    
    print("="*70)
    print("NetCDF File Comparison Tool - PERCENT DIFFERENCE")
    print(f"Undefined data threshold: {UNDEFINED_VALUE:.2e}")
    print("="*70)
