...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "-v", "--version",
        type=v.al.VERSIONS, default="v2", metavar="{v1|v2}",
        help=f"GEOS-CF Model Version. {v.al.COLLECTIONS.help_suffix()}",
    )
    parser.add_argument(
        "-c", "--collection",
        type=v.al.COLLECTIONS, default="fcast", metavar="{fcast|assim}",
        help=f"Forecast or assimilation stream. {v.al.COLLECTIONS.help_suffix()}",
    )
    parser.add_argument(
        "-d", "--dataset",
        type=v.al.DATASETS, default="aqc", metavar="{chm|aqc|met}",
        help=f"Target dataset. {v.al.DATASETS.help_suffix()}",
    )
    parser.add_argument(
        "-l", "--level",
        type=v.al.LEVELS, default="slv", metavar="{slv|p23|v72}",
        help=f"Vertical choice. {v.al.LEVELS.help_suffix()}",
    )
    parser.add_argument("-lat", "--lat", type=v.lat_type, default=38.9, help="Latitude [-90, 90].")
    parser.add_argument("-lon", "--lon", type=v.lon_type, default=-77.0, help="Longitude [-180, 180].")
    parser.add_argument(
        "-s", "--start", type=v.date_type_allow_latest, default=None,
        help="Model start date (YYYYMMDD) or 'latest'.",
    )
    parser.add_argument(
        "-e", "--end", type=v.date_type_allow_latest, default=None,
...
def build_legacy_parser(script: Optional[str] = None, program: Optional[str]=None) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-P", "--product",
        type=v.al.LEGACY_PRODS, default="default",
...
def build_parser(script: Optional[str]=None, program: Optional[str]=None) -> argparse.ArgumentParser:
    script = script or SCRIPT
    program = program or 'python'
    examples = [
        'Examples:',
