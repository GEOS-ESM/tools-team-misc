...
def format_fill_value_scientific(obj, fill_value: float = 1.0e15, fmt: str = "%.1e"):
    if isinstance(obj, float):
        ...
        return {
            k: format_fill_value_scientific(v, fill_value, fmt) for k, v in obj.items()
        }
...
class fileHelper(versionConfig):
    ...
    def _get_dt(
        self, s: Optional[str | dt.datetime], default: Optional[dt.datetime] = None
    ...
    def get_date_range(self) -> Tuple[dt.datetime, dt.datetime]:
        date_limits: List = self.data.get("date_limits", {}).get(
            self.cfg.collection, [None, None]
        )
    ...
    def get_file_list(
        self, data_limit_override: bool = False, datakey: Optional[str] = None
    ) -> List[str]:
    ...
    def get_assim_file_list(
        self, data_limit_override: bool = False, time_period: int = 1
    ) -> List[str]:
        period = dt.timedelta(
            time_period + 1
        )  # default to 1 day (capture 2 days to grab last 24 hours)
        ...
        if not end and not start:
            ...
            if (
                del_time > dt.timedelta(32)
                and "aqc" not in self.stream_name
                and not data_limit_override
            ):
                raise BadRequest(
                    f"Requested time period ({del_time.days} days) too long. Period should be < 30 days"
                )
                # return [], 'Over data limit'
            if del_time > dt.timedelta(92):
                raise BadRequest(
                    f"Requested time period ({del_time.days} days) too long. Period should be < 90 days"
                )
        cur = start
        file_list: List[str] = []
        while cur <= end:
    ...
    def get_index_by_time(self, file_list, time_period, time_flag):
        ...
        if time_flag == "end":
            flist_out = [
                f
                for f, fdt in zip(file_list, dt_list)
                if (dt_list[-1] - fdt) <= dt.timedelta(days=time_period)
            ]
        elif time_flag == "start":
            flist_out = [
                f
                for f, fdt in zip(file_list, dt_list)
                if (fdt - dt_list[0]) <= dt.timedelta(days=time_period)
            ]
        else:
            flist_out = file_list
        return flist_out
    ...
    def get_keys(
        self, datakey: Optional[str] = None, products: Optional[List] = None
    ) -> Tuple[dict, dict]:
...
def _get_init_points(
    ...
    with xr.open_dataset(file, decode_times=decode_times, engine=engine) as ds0:
        ...
        return dataIndex(
            nlevs=nlevs, lon=near_lon, lat=near_lat, t0=t0, longnames=longnames
        )
...
class cacheHelper:
    ...
    def get_schema(
        ...
        return {"longname": self.d0.longnames, **ds_schema, **base_schema}
