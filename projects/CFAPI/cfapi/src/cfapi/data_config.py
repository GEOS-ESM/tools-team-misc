...
class fileHelper(versionConfig): 
    ...
    def get_fcsts(self, dataset: str):
        ...
        return [dt.datetime.strptime(f.name,ft) for f in sorted(fcsts.glob(f'{collection}*'))]
    ...
    def get_index_by_time(self, file_list, time_period, time_flag):
        ...
        dt_list = [dt.datetime.strptime(f.rsplit('.',3)[fi],'%Y%m%d_%H%Mz') for f in file_list]
        if time_flag=='end':
