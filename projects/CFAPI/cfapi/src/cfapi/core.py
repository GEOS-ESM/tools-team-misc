...
def pool_get_interp_dat(files, di: dataIndex, keys, mp_pool=None, *, engine="netcdf4"):
    ...
    target = ft.partial(
        get_interp_dat, keys=keys_in, near_lat=di.lat, near_lon=di.lon, engine=engine
    )
...
def safe_pool_interp_data(files, d0, keys, pool, *, engine="netcdf4"):
    keys_in = [d.get("var") for d in keys]
    target = ft.partial(
        get_interp_dat, keys=keys_in, near_lat=d0.lat, near_lon=d0.lon, engine=engine
    )
...
def df2dict(
    df: pd.DataFrame, products: List[str], fill_value: Optional[str | float] = None
...
def dict2df(
    ...
    if products is not None:
        prod_keys = [
            k for k in data.keys() if k.lower() in [p.lower() for p in products]
        ]
    else:
    ...
    # -------------
        ...
        dframe = {key: data[key] for key in prod_keys}
...
def build_response_raw(**kwargs) -> Dict[str, Any]:
    ...
    out = build_response_cfg(cfg=cfg, **kwargs)
    return out


def build_response_cfg(cfg: CliConfig, **kwargs) -> Dict[str, Any]:
    """
    ...
    d0 = _get_init_points(
        ...
        cfg.lat,
        cfg.lon,
        engine=kwargs.get("netcdf_engine"),
    )
    # Optional cache path
    ...
    output = get_data(
        file_list, d0, helper.product_keys, kwargs.get("mp_pool"), fill_value=fill_value
    )
...
def get_multifile_vars(cfgs: List[CliConfig], **kwargs) -> Dict[str, Any]:
    t0 = time.time()
    fill_value = kwargs.get("fill_value", MISSING)
    file_loc = kwargs.get("file_loc", False)
    helper = fileHelper(cfgs[0], kwargs.get("config_data"), kwargs.get("config_params"))
    # File list
    fl_args = dict(data_limit_override=kwargs.get("data_limit_override", False))
    cache_args = dict(
    ...
    with mp.Pool(processes=8) as pool:
        ...
        for cfg in cfgs:
            ...
            output = get_data(
                file_list, d0, keys, pool, engine="netcdf4", fill_value=fill_value
            )
            schema = cache.get_schema(
                output.times, output.levs, helper, cfg.datakey, cfg.products
            )
