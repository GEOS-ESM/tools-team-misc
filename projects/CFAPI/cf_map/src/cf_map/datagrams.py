...
import logging
from typing import (List, Generator, Tuple, Dict, Optional, Any, 
                    Mapping, Iterable, Literal)
...
class CFDatagram:
    def __init__(self, 
                 data_prepper, lat, lon, plot_type='pres', **kwargs):
    ...
    def make_plot(self, product: Literal['co','no2','pm25','o3','so2']='no2'):
        ...
        self.add_logos()
        filename = file_template.format(version=self.version,type='pres',product=product,
                                        lat=data.lat,lon=data.lon,ext='png')
    ...
    def plot_cdtt(self, ax, met, tavg=3):
        ...
        ax.bar(met.time[::tavg], met.cldtt[::tavg], width=width, color='w', align='center', linewidth=linewidth)
        ax.yaxis.set_ticks([])
    ...
    def plot_met(self,ax, met):
        ax.set_xlim([met.time[0], met.time[-1]])
        ax.bar(met.time, met.precip, color='b', width=width*.5, align='center', linewidth=linewidth, alpha=0.5)
        ...
        ax2.set_ylabel(r'Temp 2m ($^\circ$F)', color='r', rotation=-90, fontsize=11, labelpad=15)
        ...
        ax3.set_ylabel('Wind 10m (m/s)', color='k', fontsize=11, rotation=-90, labelpad=15)
    ...
    def plot_sfc_pm(self,ax,chm,product):
        ...
        plot_val = totals
        for b in bars:
            ax.bar(chm.time, plot_val, color=b.color, width=width*.2, align='center', linewidth=linewidth)
            # self.fig.text(xlabel, ylabel, b.label, va='center', rotation='vertical', fontsize=11, color=b.color)
            ax.text(
                xlabel, ylabel, b.label,
                va='center', rotation='vertical', fontsize=11, color=b.color,
    ...
    def plot_pres_contour(self,ax, data, product):
        ...
        ax.invert_yaxis()
        CS = ax.contour(data.time,data.levs,field, colors='k', levels=CFG.scale, linewidths=0.4, linestyles='-')
        ...
        cbar = self.fig.colorbar(ax_img, cax=cbar_ax, orientation='vertical', format=CFG.cb_fmt, ticks=CFG.scale) 
        cbar.ax.yaxis.set_tick_params(pad=40,direction='in')
        cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(),ha='right')
        cbar.set_label(CFG.label, fontsize=11, fontweight='bold', rotation=270, labelpad=25)
        try:
