...
class plotConfig:
    ...
    def _load_field(self, cfg):
        ...
        if isinstance(scale, dict):
            scale = range(
                scale.get("min", 0.01), scale.get("max", 150), scale.get("step", 5)
            )
        return SimpleNamespace(
            ...
            colormap=cm,
            label=cfg.get("label"),
            cb_fmt=cfg.get("cb_format"),
            contour_fmt=cfg.get("contour_fmt"),
        )
    ...
    def get_prod_config(self, product):
        return next(
            (v for k, v in self.config.items() if product.lower() == k.lower()), {}
        )
