...
def build_plot_parser(
    ...
    parser.add_argument(
        "-o",
        "--output_dir",
        type=lc.directory_type,
        default=lc.IMG_OUTPUT_DIR,
        help=f"Output Directory. Defaults to subdirectory of plots from current dir",
    )
    return parser


def build_file_maker_parser(
    script: Optional[str] = None, program: Optional[str] = None
) -> argparse.ArgumentParser:
    script = script or SCRIPT
    program = program or "python"
    ...
    parser.add_argument(
        ...
        type=lc.DATASETS,
        default="chm_slv",
        metavar="{chm_slv|chm_p23|met_slv}",
