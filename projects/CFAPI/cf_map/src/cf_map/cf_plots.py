...
class CFSurfacePlot(CFDatagram):
    ...
    def make_plot(self, product: Optional[str]=None,data: Optional[dict]=None):
        ...
        ax1 = plt.subplot2grid((28, 1), (1, 0), rowspan=2,)
        ...
        self.add_logos()
        filename = file_template.format(version=self.version,type='sfc', product=product,
...
class CFReplayPlot(CFDatagram):
    ...
    def make_plot(self, product: Optional[str]=None,data: Optional[dict]=None):
        ...
        self.add_labels(data.met.time,data.lat,data.lon)
        self.add_logos()
        filename = file_template.format(version=self.version,type='replay', product=product,
                                        lat=data.lat,lon=data.lon, ext='png')
        output_dir = getattr(self,'output_dir') or IMG_OUTPUT_DIR
        output_dir = output_dir / f"{data.met.time[0].strftime('%Y%m%dT%H%M')}_{data.met.time[-1].strftime('%Y%m%dT%H%M')}"
        file_out = output_dir / filename
        file_out = self.save_plot(file_out)
        return file_out
    def set_xticks(self, ax,times):
        # ax.xaxis.set_ticks([t for t in times if t.hour == 0 or t.hour == 12])
        ax.xaxis.set_ticks([t for t in times if (t - times[0]).days % 5 == 0 and t.hour==0])
        return ax
    
    def add_labels(self, times, lat, lon, station: Optional[str]=''):
        self.fig.suptitle(f'GEOS CF Replay  \n {times[0].strftime('%Hz %m/%d/%Y')} - {times[-1].strftime('%Hz %m/%d/%Y')}', fontsize=14, fontweight='bold', y=1.02)
