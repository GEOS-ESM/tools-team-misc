...
def num_days_type(s: str) -> float:
    try:
        ...
        raise argparse.ArgumentTypeError(f"Number of days (days) must be a float or integer: {s}") from e
    if val > 31:
        raise argparse.ArgumentTypeError(f"Number of days (days) is limited to 1 month (31 days): {val}")
    return val
...
def plot_file_type(s: str) -> Path:
    ...
    if val.suffix.lower() not in ['png','jpg','jpeg']:
        raise argparse.ArgumentTypeError(f"Output file {s} must have a figure extension")
...
PLOTTYPES = make_aliases_from_dict(
    ...
    'replay': ['assim','historical','hist','rply','replay','assimilation','ana','analysis'],
...
def coerce_inputs(raw: Mapping[str, Any], 
                type: Optional[Literal['plot','file']]=None)-> argparse.Namespace:
