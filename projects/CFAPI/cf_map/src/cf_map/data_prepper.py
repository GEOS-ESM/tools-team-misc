...
def api_call(cfg: CliConfig):
    ...
    response = requests.get(f'https://fluid.nccs.nasa.gov/cf/api/{cfg.collection}', params=params)
    data = response.json()   
    return data
...
class CFDatagramPrep:
    ...
    def __repr__(self):
        ...
        return (f"<CFDatagramPrep plot='{plot_type}' version='{version}' "
    ...
    def get_plot_vars(self, 
        ...
        if plot_type == 'replay':            
            ...
            groups += [dict(datakey='aqc_slv',products=['NO2','O3','SO2','PM25','CO'])]
        else:
            groups += [dict(datakey='chm_slv',products=['NO2','O3','SO2','PM25','CO','PM25_RH35'])]
        
            if plot_type == 'pres':
                groups += [dict(datakey='chm_p23',products=['NO2','O3','SO2','PM25','CO'])]
    ...
    def prep_met(self,data: dict) -> SimpleNamespace:
        ...
        return SimpleNamespace(
            wspd = calc_wspd(get_wbackup(vals,['U10M','U'],[]),get_wbackup(vals,['V10M','V'],[])),
            winddir = calc_winddir(get_wbackup(vals,['U10M','U'],[]),get_wbackup(vals,['V10M','V'],[])),
    ...
    def prep_contour_data(self, data: dict, min_P: float = 500.0) -> SimpleNamespace:
        def get_2d_array(vals, lev_keys, keys, default=[]):
            ...
            return np.array([data.get(lev_keys.get(k)) for k in sorted(lev_keys.keys())])
        vals = data.get('values',{})
