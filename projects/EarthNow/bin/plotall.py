...
def parse_args():
    parser = argparse.ArgumentParser(description="Generic WxMaps plotting driver")
    ...
    parser.add_argument(
        "--product", required=True, choices=PRODUCTS.keys(), help="Plot product type"
    )

    parser.add_argument(
        "--map-type", required=True, help="Map domain (e.g. conus, europe)"
    )
    ...
    parser.add_argument(
        "--nproc", type=int, default=1, help="Number of parallel workers"
    )
    parser.add_argument(
        "--compress-level",
        type=int,
    ...
    parser.add_argument("--exp-id", default="Feature-c2160_L137", help="Experiment ID")
    parser.add_argument(
        "--collection",
        default="inst1_2d_asm_Nx",
...
def main():
    ...
    if isinstance(map_config.projection, ccrs.Geostationary):
        ...
        fig_temp = plt.figure(
            figsize=resolution_config.figsize, dpi=resolution_config.dpi
        )
