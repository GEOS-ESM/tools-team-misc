...
def plot_dust_rgb(fig, ax, plotter, reader, args):
    ...
    data, lats, lons, meta = reader.read_variable(
        args.fdate, args.pdate, variables=["TBRB08RG"]
    )
    ...
    print(
        f"R: min={np.nanmin(red_raw):.6f}, max={np.nanmax(red_raw):.6f}, mean={np.nanmean(red_raw):.6f}"
    )
    print(
        f"G: min={np.nanmin(grn_raw):.6f}, max={np.nanmax(grn_raw):.6f}, mean={np.nanmean(grn_raw):.6f}"
    )
    print(
        f"B: min={np.nanmin(blu_raw):.6f}, max={np.nanmax(blu_raw):.6f}, mean={np.nanmean(blu_raw):.6f}"
    )
    ...
    print(
        f"R: min={np.nanmin(R):.6f}, max={np.nanmax(R):.6f}, mean={np.nanmean(R):.6f}"
    )
    print(
        f"G: min={np.nanmin(G):.6f}, max={np.nanmax(G):.6f}, mean={np.nanmean(G):.6f}"
    )
    print(
        f"B: min={np.nanmin(B):.6f}, max={np.nanmax(B):.6f}, mean={np.nanmean(B):.6f}"
    )
