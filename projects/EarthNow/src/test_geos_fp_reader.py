...
def test_reader_initialization():
    ...
    try:
        ...
        reader = ReaderClass(
            base_path="/discover/nobackup/projects/gmao/gmao_ops/pub", exp_id="f5295_fp"
        )
...
def test_find_available_times(reader):
    ...
    try:
        ...
        print(
            f"  Last 5 times:  {[t.strftime('%Y%m%d_%H%M') for t in times_inst[-5:]]}"
        )
    ...
    for varname in test_vars:
        try:
            ...
            data, lats, lons, meta = reader.read_variable(
                fdate=fdate, pdate=pdate, variables=varname, var_type="inst"
            )
...
def test_read_3d_variable(reader):
    ...
    try:
        ...
        data, lats, lons, meta = reader.read_variable(
            fdate=fdate, pdate=pdate, variables="T", var_type="pres", level=500.0
        )

        print(f"✓ Successfully read T at 500 hPa")
        print(f"  Data shape: {data.shape}")
        print(
            f"  Data range: [{data.min():.2f}, {data.max():.2f}] {meta.get('units', '')}"
        )
...
def test_read_multiple_variables(reader):
    ...
    try:
        ...
        results = reader.read_multiple_variables(
            fdate=fdate, pdate=pdate, variables=var_list, var_type="inst"
        )
        ...
        for varname, (data, lats, lons, meta) in results.items():
            ...
            print(
                f"    Range: [{data.min():.4f}, {data.max():.4f}] {meta.get('units', '')}"
            )
            print(f"    Long name: {meta.get('long_name', 'N/A')}")
...
def test_list_available_cycles(reader):
    ...
    try:
        ...
        cycles = reader.list_available_cycles(
            start_date="2026-01-20", end_date="2026-01-23"
        )
