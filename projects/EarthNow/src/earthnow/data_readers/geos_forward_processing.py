...
class GEOSForwardProcessingReader:
    ...
    INST_COLLECTION = (
        "inst_30mn_met_c0720sfc"  # High-res 30-minute instantaneous (cubed-sphere)
    )
    TAVG_COLLECTIONS = [
    ...
    def _get_target_latlon_grid(self, cubed_sphere_nx=None):
        ...
        if self.target_resolution == "auto":
            ...
            if cubed_sphere_nx is None:
                raise ValueError(
                    "Cannot use 'auto' resolution without cubed_sphere_nx parameter"
                )
    ...
    def _regrid_cubed_sphere_to_latlon(self, data_tiles, cs_lons, cs_lats):
        ...
        if data_tiles.ndim == 3 and data_tiles.shape[0] == 6:
            ...
            raise ValueError(
                f"Unexpected data shape: {data_tiles.shape}. Expected (6, nx, nx)"
            )

        # Get target grid (will auto-detect if resolution="auto")
        target_lats, target_lons = self._get_target_latlon_grid(
            cubed_sphere_nx=cubed_sphere_nx
        )
        ...
        print(
            f"  Interpolating {len(data_flat)} cubed-sphere points to {len(target_lats)}x{len(target_lons)} lat-lon grid..."
        )
    ...
    def read_variable(
        self, fdate: str, pdate: str, variables, var_type="inst", level=None
    ):
        """
        ...
        for varname in variables:
            ...
            if varname.startswith("VORT") and len(varname) > 4:
                try:
                    ...
                    print(
                        f"Detected computed variable: {varname} (vorticity at {vort_level} hPa)"
                    )
            ...
            if varname in SURFACE_FIELDS_IN_PRES:
                ...
                try:
                    ...
                    print(
                        f"  Could not find {varname} in pressure file, trying other collections..."
                    )
                    continue
            ...
            if extracted_var and extracted_level:
                ...
                try:
                    ...
                    print(
                        f"Detected level extraction: {varname} → {extracted_var} at {extracted_level} hPa"
                    )
                    try:
                        ...
                        print(
                            f"  Could not find {extracted_var} at {extracted_level} hPa, trying next variable..."
                        )
                        continue
        ...
        with Dataset(path) as nc:
            ...
            if is_3d and level is not None:
                if pressure_levels is None:
                    raise ValueError(
                        f"Cannot extract level {level}: no pressure levels found"
                    )
                ...
                print(
                    f"Extracting level {level} hPa (nearest: {actual_level:.2f} hPa, index {level_idx})"
                )
            ...
            if is_cubed_sphere:
                ...
                if data.ndim != 3 or data.shape[0] != 6:
                    raise ValueError(
                        f"Unexpected cubed-sphere data shape: {data.shape}. Expected (6, nx, nx)"
                    )
                ...
                lats, lons = self._get_target_latlon_grid(
                    cubed_sphere_nx=cubed_sphere_nx
                )
    ...
    def _get_precip_type(self, fdate: str, pdate: str, ptype: str):
        ...
        if ptype == "RAIN":
            ...
            print(
                f"  RAIN range: [{rain.min():.2e}, {rain.max():.2e}] {metadata['units']}"
            )
    ...
    def _read_precip_variable(self, fdate: str, pdate: str, variables: List[str]):
        ...
        with Dataset(path) as nc:
            ...
            if is_cubed_sphere:
                ...
                if data.ndim != 3 or data.shape[0] != 6:
                    raise ValueError(
                        f"Unexpected cubed-sphere data shape: {data.shape}"
                    )
            ...
            print(
                f"  Data range: [{data.min():.2e}, {data.max():.2e}] {metadata['units']}"
            )
    ...
    def read_variable(
        ...
        for varname in variables:
            ...
            if varname in ACCUMULATION_VARIABLES:
                ...
                try:
                    return self._compute_accumulation(
                        fdate, pdate, base_var, scale_factor
                    )
                except FileNotFoundError as e:
                    print(f"  Could not compute {varname}: {e}")
                    continue
            ...
            if extracted_var and extracted_level:
                ...
                try:
                    ...
                    print(
                        f"  Found {varname} directly in {collection}, not doing level extraction"
                    )
                    ...
                    print(
                        f"  {varname} not found directly, trying level extraction: {extracted_var} at {extracted_level} hPa"
                    )
                    try:
                        return self.read_variable(
                            fdate=fdate,
                        ...
                        print(
                            f"  Could not extract {extracted_var} at {extracted_level} hPa, trying next variable..."
                        )
                        continue
    ...
    def _compute_accumulation(
        self, fdate: str, pdate: str, variable: str, scale_factor: float = 1.0
    ):
        """
        ...
        if start_dt >= end_dt:
            ...
            try:
                _, lats, lons, meta = self.read_variable(
                    fdate=fdate, pdate=fdate, variables=["PHIS"], var_type="inst"
                )
            except Exception as e:
                raise FileNotFoundError(
        ...
        for i, time_dt in enumerate(valid_times):
            ...
            try:
                data, lats, lons, meta = self.read_variable(
                    fdate=fdate, pdate=time_str, variables=[variable], var_type="tavg"
                )
            except FileNotFoundError:
            ...
            print(
                f"    Δt = {dt_hours:.2f} hrs, increment range: [{increment_mm.min():.3f}, {increment_mm.max():.3f}] mm"
            )

        if accumulated is None:
            raise FileNotFoundError(f"No valid data found for {variable} accumulation")
        ...
        print(
            f"    Accumulated range: [{accumulated.min():.3f}, {accumulated.max():.3f}] mm"
        )
        if scale_factor != 1.0:
    ...
    def get_available_variables(
        self, fdate: str, pdate: str, var_type="inst"
    ) -> List[str]:
        """
    ...
    def compute_vorticity(self, fdate: str, pdate: str, level: float = 500.0):
        ...
        u_data, lats, lons, u_meta = self.read_variable(
            fdate=fdate, pdate=pdate, variables=["U"], var_type="pres", level=level
        )

        v_data, _, _, v_meta = self.read_variable(
            fdate=fdate, pdate=pdate, variables=["V"], var_type="pres", level=level
        )
    ...
    def _compute_vorticity_latlon(self, u, v, lats, lons):
        ...
        dv_dx[:, 1:-1] = (v[:, 2:] - v[:, :-2]) / (
            2 * dlon * R_earth * np.cos(lat_mesh[:, 1:-1])
        )
        dv_dx[:, 0] = (v[:, 1] - v[:, -1]) / (
            2 * dlon * R_earth * np.cos(lat_mesh[:, 0])
        )  # Periodic BC
        dv_dx[:, -1] = (v[:, 0] - v[:, -2]) / (
            2 * dlon * R_earth * np.cos(lat_mesh[:, -1])
        )  # Periodic BC
        ...
        du_dy[0, :] = (u[1, :] - u[0, :]) / (
            dlat * R_earth
        )  # Forward difference at pole
        du_dy[-1, :] = (u[-1, :] - u[-2, :]) / (
            dlat * R_earth
        )  # Backward difference at pole
    ...
    def _get_times_from_collection(
        self, forecast_dir: str, collection: str
    ) -> List[datetime]:
        """
        ...
        pattern = os.path.join(forecast_dir, f"GEOS.fp.fcst.{collection}.*.V01.nc4")
