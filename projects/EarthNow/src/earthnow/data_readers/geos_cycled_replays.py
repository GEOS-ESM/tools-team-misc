...
class GEOSDataReader:
    ...
    def __init__(
        ...
        self.forecast_base = os.path.join(exp_path, exp_res, exp_id, "forecasts")
    ...
    # ------------------------------------------------------------------
    def get_file_path(
        self, fdate: str, pdate: str, raise_on_missing=False
    ) -> Optional[str]:
        """
    ...
    def read_multiple_variables(
        self, fdate: str, pdate: str, variables: List[str], raise_on_missing=False
    ) -> Dict[str, Tuple]:
        """
        ...
        """
        path, collection, _ = self.resolve_file(
            fdate, pdate, raise_on_missing=raise_on_missing
        )
    ...
    def get_available_variables(
        ...
        with Dataset(path) as nc:
            return sorted(
                v for v in nc.variables if v not in ("lat", "lon", "time", "lev")
            )
    ...
    def find_available_times(self, fdate: str) -> List[datetime]:
        ...
        for forecast_dir in self._forecast_dirs(fdate):
            ...
            for collection in self._candidate_collections():
                pattern = os.path.join(forecast_dir, f"GEOS.{collection}.*.nc4")
