...
class GenCastGEOSFPReader:
    ...
    def __init__(
        ...
        self.data_dir = os.path.join(exp_path, self.fp_id, exp_res, "2-predictions")
        ...
        for varname in variables:
            if varname in ["PRECACCUM", "SNOWACCUM"]:
                return self._compute_accumulation(
                    fdate, pdate, varname, ensemble_member
                )
            ...
            if varname.startswith("VORT") and len(varname) > 4:
                try:
                    ...
                    return self.compute_vorticity(
                        fdate, pdate, level=vort_level, ensemble_member=ensemble_member
                    )
                except ValueError:
        ...
        for varname in variables:
            ...
            try:
                with Dataset(path) as nc:
                    ...
                    print(f"Ensemble member : {ensemble_member}")
                    print(
                        f"Time index      : {time_idx} (+{metadata['forecast_hour']}h)"
                    )
                    if is_3d:
                        print(f"Pressure level  : {actual_level} hPa")
                    print(f"Data shape      : {data.shape}")
                    print(
                        f"Data range      : [{np.nanmin(data):.2e}, {np.nanmax(data):.2e}]"
                    )
    ...
    def compute_vorticity(
        self, fdate: str, pdate: str, level: float = 500.0, ensemble_member=None
    ):
        """
    ...
    def _compute_accumulation(
        self, fdate: str, pdate: str, varname: str, ensemble_member=None
    ):
        """
        ...
        if forecast_hour > 240:
            raise ValueError(
                f"GenCast only has forecasts to +240h, requested +{forecast_hour}h"
            )
        ...
        with Dataset(path) as nc:
            ...
            for hour in timestep_hours:
                ...
                print(
                    f"    +{hour}h: increment range [{np.nanmin(data):.3f}, {np.nanmax(data):.3f}]"
                )
        ...
        print(
            f"    Total period: {last_timestep_hour} hours ({num_timesteps} timesteps)"
        )
        print(
            f"    Accumulated range: [{np.nanmin(accumulated):.3f}, {np.nanmax(accumulated):.3f}] mm"
        )
...
def create_reader(**kwargs):
    ...
    Usage:
        from earthnow.gencast_geos_fp import create_reader
