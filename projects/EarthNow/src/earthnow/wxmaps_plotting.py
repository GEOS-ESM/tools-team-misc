...
# functions
# I actually don't need to infer the grid for imshow?
def infer_ostia_grid(nx, ny):
...
def get_seaice_map(year, month, day, hour):
    ...
    with FortranFile(filename, "r") as f:
        ...
        lats, lons = infer_ostia_grid(nx, ny)
        ...
        while True:
            try:
                ...
                # It's not reading the target date hmmm
                print(
...
class WxMapPlotter:
    ...
    def __init__(
        ...
        print(
            f"Initialized {self.resolution_config.name} plotter: "
    ...
    def _add_seaice_overlay(self, pdate_str: str):
        ...
        sice, slats, slons = get_seaice_map(year, month, day, hour)

        # Apply threshold — zero out low concentrations
        sice[sice < self.style.seaice_threshold] = (
            0  # Has to be zero for RGBA calc below
        )
    ...
    def _add_gshhs_background(self):
        ...
        try:
            from earthnow.wxmaps_gshhs import GSHHSReader, get_gshhs_resolution_file
            import matplotlib.patches as mpatches

            # Get GSHHS file path
            gshhs_file = get_gshhs_resolution_file(
                self.style.gshhs_path, self.style.gshhs_resolution
            )

            print(f"  Loading GSHHS from: {gshhs_file}")
            print(
                f"  Resolution: {self.style.gshhs_resolution} (min area: {self.style.gshhs_min_area} km²)"
            )
            ...
            if len(test_polygons) == 0:
                ...
                raise FileNotFoundError(
                    f"GSHHS file appears empty or unreadable: {gshhs_file}"
                )
            ...
            if len(polygons) == 0:
                print(
                    "  WARNING: No GSHHS polygons found in map extent! Falling back to Cartopy."
                )
                self.ax.add_feature(
                    cfeature.OCEAN, facecolor=self.style.ocean_color, zorder=0
                )
    ...
    def _add_gridlines(self):
        ...
        fontsize_scale = {"hd": 10, "fhd": 12, "2k": 14, "4k": 16, "8k": 24}

        fs = (
            fontsize_scale.get(self.resolution, 10)
    ...
    def add_cities(
        ...
        """
        marker_scale = {"hd": 5, "fhd": 6, "2k": 7, "4k": 10, "8k": 15}

        fontsize_scale = {"hd": 8, "fhd": 9, "2k": 10, "4k": 12, "8k": 18}
    ...
    def add_nws_warnings(self, valid_time: datetime):
        ...
        try:
            ...
            if (
                hasattr(self.style, "nws_custom_type")
                and hasattr(self.style, "nws_custom_status")
                and self.style.nws_custom_type
                and self.style.nws_custom_status
            ):
                # Plot specific custom warning type/status
                print(
                    f"Plotting custom warning: {self.style.nws_custom_type}.{self.style.nws_custom_status}"
                )
                nws.plot_warnings(
                    self.ax,
                ...
                hasattr(self.style, "nws_warning_types")
                and self.style.nws_warning_types
            ):
                ...
                for warning_type, warning_status in self.style.nws_warning_types:
                    ...
                    status_name = nws.VTEC_SIGNIFICANCE.get(
                ...
                print(
                    f"Plotting SEVERE warnings only (Tornado + Severe Thunderstorm) at {valid_time}"
                )
    ...
    def add_city_data_values(self, data, lons, lats, text_color="white"):
        ...
        city_spacing_degrees = (
            np.clip(0.5 / scale_factor, 0.2, 2.0) * spacing_multiplier
        )
    ...
    def add_city_temperatures(self, data, lons, lats, temperature_unit="F"):
        ...
        if lons.ndim == 1 and lats.ndim == 1:
            ...
            if data.shape[0] != len(lats) or data.shape[1] != len(lons):
                raise ValueError(
                    f"Data shape {data.shape} doesn't match lat/lon arrays ({len(lats)}, {len(lons)})"
                )
    ...
    def _is_within_bounds(self, lon, lat):
        ...
        extent = self.ax.get_extent(crs=ccrs.PlateCarree())
        result = extent[0] <= lon <= extent[1] and extent[2] <= lat <= extent[3]
    ...
    def add_title(self, title: str, loc: str = "center"):
        ...
        fontsize_scale = {"hd": 16, "fhd": 18, "2k": 20, "4k": 24, "8k": 36}

        fs = (
            self.style.title_fontsize
            if self.style.title_fontsize
            else fontsize_scale.get(self.resolution, 16)
        )

        self.ax.set_title(title, fontsize=fs, loc=loc, fontweight="bold", pad=20)

    def add_forecast_timestamp(
        self,
        fdate: str,
        pdate: str,
        exp: str = "CONUS02KM",
        ...
        fs = (
            self.style.timestamp_fontsize
            if self.style.timestamp_fontsize
            else fontsize_scale.get(self.resolution, 12)
        )
    ...
    def save(self, filename: str, optimize: bool = True):
        ...
        print(
            f"  Resolution: {self.resolution_config.width}x{self.resolution_config.height}"
        )
