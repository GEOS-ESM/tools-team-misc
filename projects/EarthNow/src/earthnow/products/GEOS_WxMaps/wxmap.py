...
def plot_wxmap(fig, ax, plotter, reader, args):
    ...
    phis, lats, lons, meta = reader.read_variable(
        args.fdate, args.pdate, variables=["PHIS", "HGT_SFC"]
    )
    phis = phis.astype(np.float32) / 9.81

    t2m, lats, lons, meta = reader.read_variable(
        args.fdate, args.pdate, variables=["T2M", "TMP_2M"]
    )
    ...
    dbz, lats, lons, meta = reader.read_variable(
        args.fdate, args.pdate, variables=["DBZ_MAX", "REFC"]
    )
    ...
    # ------------------------------------------------------------------
    mask = (thck > thLow) & (thck < thHigh) & ((snow > eps) | (mix > eps))
    mix[mask] = snow[mask]
    snow[mask] = 0.0
    ...
    # ------------------------------------------------------------------
    frzr_dom = (frzr > mix + eps) & (frzr > snow + eps)
    snow_dom = (snow > frzr + eps) & (snow > mix + eps)
    # Mix wins ties or near-ties
    mix_dom = ~(frzr_dom | snow_dom) & (mix > eps)
    frzr[~frzr_dom] = 0.0
    snow[~snow_dom] = 0.0
    mix[~mix_dom] = 0.0
...
def generate_colorbar():
    ...
    from earthnow.wxmaps_utils import save_colorbar_grid
