...
def append_vars(ncin, options, zlib=False):
    ...
    no2 = ncin.variables['NO2'][:,:,:,:]*46.0/28.97
    ...
    # total column NO2
    # ---------------
    this = nc.createVariable('tcno2','f4',('time', 'lat', 'lon'),zlib=zlib)
    this.long_name = 'total column NO2'
    this.missing_value = UNDEF # out of domain
    this.units = 'kg m-2'
    for n in range(nt):
        tcno2 = npy.sum(no2[n,:,:,:]*delp[n,:,:,:]*100.0/g, axis=0)
        this[n,:,:] = tcno2[:,:]
