...
#---
#def getTyme(ncin):
#    t = ncin.variables['time'][:,0,:] #TFLAG:units = "<YYYYDDD,HHMMSS>" ;
#    nt, ns = t.shape
...
def getTyme(ncin):
    t = ncin.variables['time'][:]  # TFLAG:units = "<YYYYDDD,HHMMSS>" ;
    nt, ns = t.shape
    day = timedelta(hours=2)
    tyme = np.zeros(nt, dtype='datetime64[s]')
    for i in range(nt):
        dstr = str(t[i, 0]).zfill(7) + str(t[i, 1]).zfill(6)
        date_str = "{}-{}-{}T{}:{}:{}".format(dstr[0:4], dstr[4:6], dstr[6:8], dstr[8:10], dstr[10:12], dstr[12:14])
        tyme[i] = np.datetime64(date_str)
