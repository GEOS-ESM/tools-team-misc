...
class Curv2LLP(object):
    ...
    def regrid2d(self,cV):
        ...
        # Fill missing latitude bands.
        # ----------------------------
        for j in range(1,ny-1):  # 1D interpolation in lat

            z = rV[j,:]
            G = (z!=MISSING)&(z!=UNDEF)
            if any(G): # This is not a missing latitude band.
                continue

            zs = rV[j-1,:]
            GS = (zs!=MISSING)&(zs!=UNDEF)
            if not any(GS): # No data in adjacent latitude band
                continue

            zn = rV[j+1,:]
            GN = (zn!=MISSING)&(zn!=UNDEF)
            if not any(GN): # No data in adjacent latitude band
                continue

            G = GS & GN
            rV[j,G] = (zs[G] + zn[G]) / 2.0
