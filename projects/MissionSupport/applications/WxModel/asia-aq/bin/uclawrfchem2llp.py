...
def append_lml(nc, zlib=False):
    ...
    """
    for v in ['o3', 'so2', 'hcho','no', 'no3', 'soila', 'no2', 'so4ai', 'so4aj', 'co', 'PM2_5_DRY', 'seas',   
       'orgpai', 'orgpaj', 'eci', 'ecj', 'iso', 'rho']:
        var = nc.variables[v]
...
def append_vars(nc, zlib=False): 
    ...
    nz, ny, nx = co.shape
    oc = nc.variables['orgpai'][0,:,:,:] + nc.variables['orgpaj'][0,:,:,:]  # ug kg-1
    bc = nc.variables['eci'][0,:,:,:] + nc.variables['ecj'][0,:,:,:]  # ug kg-1
    dz = nc.variables['dz'][0,:,:,:]  # m 
    no2 = nc.variables['no2'][0,:,:,:]
    no2 = no2/1e6*46.0/28.97
    no2 = no2*6.022*10e23/460
    hcho = nc.variables['hcho'][0,:,:,:]
    hcho = hcho/1e6*30.03/28.97
    nz, ny, nx = no2.shape
    nz, ny, nx = hcho.shape
    rho = nc.variables['rho'][0,:,:,:]  # kg m-3 
    pm25 = nc.variables['PM2_5_DRY'][0,:,:,:]  # ug m-3
    ext = nc.variables['EXTCOF55'][0,:,:,:]

    #AOD
    #---------------
    this = nc.createVariable('aod', 'f4',('Time', 'south_north', 'west_east'),zlib=zlib)
    this.description = 'aod'
    this.missing_value = UNDEF  
    this.units = ''
    this[0,:,:] = npy.sum(ext*dz/1000, axis=0)
    ...
    # total cloumn BC
    #----------------
    this = nc.createVariable('tcbc','f4',('Time', 'south_north', 'west_east'),zlib=zlib)
    this.description = 'total column BC'
    this.missing_value = UNDEF # out of domain
    this.units = 'kg m-2'
    this[0,:,:] = npy.sum(bc*rho*dz/1e9, axis=0) # kg m-2
    ...
    # total cloumn NO2
    #----------------
    this = nc.createVariable('tcno2','f4',('Time', 'south_north', 'west_east'),zlib=zlib)
    this.description = 'total column NO2'
    this.missing_value = UNDEF # out of domain
    this.units = 'molecules cm-2'
    this[0,:,:] = npy.sum(no2*rho*dz, axis=0) # molecules cm-2

    # total cloumn NO2
    #----------------
    this = nc.createVariable('tchcho','f4',('Time', 'south_north', 'west_east'),zlib=zlib)
    this.description = 'total column HCHO'
    this.missing_value = UNDEF # out of domain
    this.units = 'kg m-2'
    this[0,:,:] = npy.sum(hcho*rho*dz, axis=0) # molecules cm-2
