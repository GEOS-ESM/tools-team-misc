...
def append_vars(nc, zlib=False): 
    ...
    no = nc.variables['no'][0,:,:,:]  # ppmv 
    no = no/1e6*30.01/28.97 # kg kg-1
    nz, ny, nx = no.shape

    no2 = nc.variables['no2'][0,:,:,:]  # ppmv
    no2 = no2/1e6*46.0/28.97 # kg kg-1
    nz, ny, nx = no2.shape

    no3 = nc.variables['no3'][0,:,:,:]  # ppmv
    no3 = no3/1e6*30.01/28.97 # kg kg-1
    nz, ny, nx = no3.shape
    ...
    # total cloumn NO 
    #----------------
    this = nc.createVariable('tcno','f4',('Time', 'south_north', 'west_east'),zlib=zlib)
    this.description = 'total column NO'
    this.missing_value = UNDEF # out of domain
    this.units = 'kg m-2'
    this[0,:,:] = npy.sum(no*rho*dz/1e9, axis=0) # kg m-2 
    ...
    this.missing_value = UNDEF # out of domain
    this.units = 'kg m-2'
    this[0,:,:] = npy.sum(no2*rho*dz, axis=0)  # kg m-2

    # total cloumn NO3
    #----------------
    this = nc.createVariable('tcno3','f4',('Time', 'south_north', 'west_east'),zlib=zlib)
    this.description = 'total column NO3'
    this.missing_value = UNDEF # out of domain
    this.units = 'kg m-2'
    this[0,:,:] = npy.sum(no3*dz/1e9, axis=0)  # kg m-2
