...
#       tyme  += [isoparser(row['time']),]
        time  = row['time']
        time0 = update_isotime(time)
        print time, time0
        tyme  += [isoparser(time0, ignoretz=True),]
        
    return ( array(lon), array(lat), array(tyme) )
    
def update_isotime(time):

  """
    To fix ISODATE when hours has 24
      (yyyy-mm-ddT24:mn:sc)
  """
  datestring_new = time
  time0 = time
  if time[12:14] == "24":
   datestring = time[0:11]
   minsec = time[15:]
#  d = dateutil.parser.parse(datestring)
   d = isoparser(datestring)
   d2 = d + timedelta(seconds=86400)
   datestring_new = (d2.isoformat())

   if time[18:20] != "60":
    datestring_new1 = datestring_new[0:14] + minsec
    datestring_new = datestring_new1
   time0 = datestring_new



   """
    To fix ISODATE when seconds has 60
      (yyyy-mm-ddThh:mn:60)
   """

  elif time[18:20] == "60":
   datestring  = time[0:17]  
#  d = dateutil.parser.parse(datestring)
   d = isoparser(datestring)
   d2 = d + timedelta(seconds=60)
   datestring_new = (d2.isoformat())
   time0 = datestring_new
  return (time0)
